...
export function StrategyDetailPage({ market }: Props) {
  ...
  useEffect(() => {
    ...
    const mkt = (definition?.universe as any)?.market as string | undefined;
    if (!tickers?.length || !mkt) return;
      ...
      <div className="min-h-screen">
        <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
          <div className="flex items-center gap-4">
            <a href={`/${market.toLowerCase()}`} className="text-sm text-blue-600 hover:text-blue-700">&larr; Back to Generator</a>
  ...
  const stratMarket = (def.universe as AnyObj)?.market as string ?? (strategy.market as string) ?? "";
  ...
  const currency = (backtestConfig.currency as string) ?? config.currency;
              ...
              <div className="rounded-xl border border-white/[0.06] bg-[#111118] p-4">
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
                  {tickerMetrics.map((s) => {
                    ...
                    const tickerCurrency = s.currency as string;
                    const symbol = tickerCurrency === "INR" ? "₹" : "$";
            ...
            {!backtest && !confidence && (
              <div className="rounded-2xl border border-white/[0.06] bg-[#111118] px-6 py-12 text-center">
                ...
                <p className="mt-1 text-xs text-gray-500">
                  ...
                  <a href={`/${market.toLowerCase()}`} className="text-blue-600 hover:text-blue-700">Generator</a>
